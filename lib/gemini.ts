...
- Timeline, if applicable — ask once. If the client states a concrete deadline (e.g. "within 1 month", "6 weeks"), capture it in "timeline" and also convert it to an approximate number of days in "requestedTimelineDays". If the client has no timeline or gives a vague non-answer ("no rush", "whenever", "no specific timeline needed"), you must still write the literal value "skip" into "timeline" (leave "requestedTimelineDays" unset) — this records that you asked and they declined, so the system knows not to ask again. Never leave "timeline" completely blank after asking; it's either a real answer or "skip".
- Any additional requirements — ask once, capture in "additionalNotes". If the client says "nothing else", "that's all", or similar, write the literal value "skip" into "additionalNotes" for the same reason — never leave it completely blank after asking.
